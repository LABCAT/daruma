...
Automated idea intake for Metal Monkey Apps. **Brainstorm** decides; Daruma schedules research.
...
## Ops refresh (not app ideation)

**Model prices** — monthly: refetch BenchLM + Artificial Analysis into [`../MODEL_COSTS.md`](../MODEL_COSTS.md). Sources listed in that file. Automate later (scheduled issue or Worker).

## Later (Phase 2)

**Daruma Opportunity Engine** — see [`../projects/opportunity-engine/`](../projects/opportunity-engine/). Batch pipeline (Collect → Score → Synthesise) replaces the paste step; posts to the issue or Brainstorm. Founder still scores Passion and picks in Brainstorm until automation is trusted.
