...
**Daruma command center (`daruma.labcat.nz`):** **SvelteKit** + `@sveltejs/adapter-cloudflare` (UI + API) — locked ([`projects/02-DASHBOARD.md`](projects/02-DASHBOARD.md))  
**Metal Monkey web (`apps.metalmonkey.cc`):** decide later (Tanuki first); same SvelteKit stack as dashboard when built
...
**Hosting:** Cloudflare Pages (legacy SvelteKit products); Cloudflare Workers for Daruma dashboard (SvelteKit adapter)
...
**Toolbox web template (legacy `web.daruma.nz`):** SvelteKit + Neon + Upstash + Cloudflare Pages + Stripe + basic auth + E2E (Playwright) — current. New web: Metal Monkey `*.metalmonkey.cc` + SvelteKit (same stack as dashboard).
